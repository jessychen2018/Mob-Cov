...
def scales_to_loc(scales_vector, comb, space=True):
    ...
    x = int(
        sum([np.floor(size_of_the_box / ticks_prod[1 + s]) * ((c) % comb[s]) for (s, c) in enumerate(scales_vector)]))
    y = int(sum([np.floor(size_of_the_box / ticks_prod[1 + s]) * np.floor((c) / comb[s]) for (s, c) in
...
def run_simulation(iterations, comb, n_scales, c0, c1, d, np, T, pos, infected):
    '''
    ...
    infected: infected particle id

    Output
    ------
    pos: particle positions for np particles at a total time of T
    infected: infected particle id at a total time of T
    '''
...
class ContainerModel():
    ...
    def __init__(self, comb, c0, c1, d, n_particles, total_time, initial_infected_number):
        self.c0 = c0
        self.c1 = c1
        self.d = d
        self.comb = comb
        ...
        self.particle_position = np.zeros((total_time, n_particles, 2), int)
        self.n_scales = len(comb)
        self.infected = [list(np.random.choice(a=range(n_particles), size=initial_infected_number))]

    def run_simulation(self, iterations):
        self.particle_position = run_simulation(iterations, self.comb, self.n_scales, self.c0, self.c1, self.d,
                                                self.n_particles, self.total_time, self.particle_position, self.infected)
